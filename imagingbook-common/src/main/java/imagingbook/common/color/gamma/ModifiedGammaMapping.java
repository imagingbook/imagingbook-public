 ...
 * function consists of a linear and non-linear part. In the forward mapping ({@link #applyFwd(double)}), the linear
 * part covers input values between a = 0,...,a0, while values a = a0,...,1 are mapped non-linearly. For the inverse
 * mapping ({@link #applyInv(double)}), the linear part is b = 0,...,b0, the non-linear part is b = b0,...,1.
 * Theoretically all mapping parameters can be derived from parameters {@code gamma} and {@code a0} (the linear to
 * non-linear transition point) only. Note that {@code gamma} specifies the nominal &gamma; parameter for the
 * <em>forward</em> (i.e., linear to non-linear) mapping, e.g., &gamma; = 1/2.4 for sRGB. See Sec. 3.7.6 of [1] for more
 * details.
 * </p>
 * <p>
...
public class ModifiedGammaMapping implements GammaMapping {
	...
	/**
	 * Gamma mapping function instance for sRGB (see Table 3.1 of [1]). Note that we need to specify parameters s and d
	 * too to comply strictly with the sRGB standard.
	 */
	...
	/**
	 * Function s(gamma, a0) derives the slope s of the linear section from parameters gamma and a0 (see Eq. 3.35 in
	 * [1]).
	 *
	...
	/**
	 * Function d(gamma, a0) derives the offset d of the non-linear section from parameters gamma and a0 (see Eq. 3.35
	 * in [1]).
	 *
	 * @param gamma the nominal gamma value for the forward mapping
	 * @param a0 the linear to non-linear transition point
