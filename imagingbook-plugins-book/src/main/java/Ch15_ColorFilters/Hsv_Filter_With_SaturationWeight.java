...
public class Hsv_Filter_With_SaturationWeight implements PlugInFilter {
	
	static double sigma = 3.0;
	static boolean FILTER_ALL_COMPONENTS = true;
	...
	public void run(ImageProcessor ip) {
		...
		for (int v = 0; v < h; v++) {
			for (int u = 0; u < w; u++) {
				...
				fHcos.setf(u, v, (float) Math.cos(theta) * s);
				fH.setf(u, v, HSV[0]);	// hue 
				fS.setf(u, v, HSV[1]);	// saturation
				fV.setf(u, v, HSV[2]);	// value
			}
		}
	...
	FloatProcessor combineCosSin(FloatProcessor fHcos, FloatProcessor fHsin) {
		...
		for (int v = 0; v < h; v++) {
			for (int u = 0; u < w; u++) {
				...
				double theta = Math.atan2(Hsin, Hcos);    // Hpp in [-pi, .. pi]
				float H = (float) (Arithmetic.mod(theta, twoPi) / twoPi);  // H in [0, 1]
				fHrec.setf(u, v, H);
			}
		}
		return fHrec;
